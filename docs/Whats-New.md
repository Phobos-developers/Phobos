...
- Fixed the bug where Jumpjet locomotor weapons (`IsLocomotor=yes`, `Locomotor=Jumpjet`) would cause `BalloonHover=yes` targets to land directly on the attacker unit upon arrival (by NetsuNegi)
