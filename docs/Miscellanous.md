...
- In vanilla, players can only issue commands to up to 128 technos at a time. This is due to the fact that unit execution requires sending events, and the event buffer length is 128.
- Now you can use the following flag to make the unit cache received instructions if the event buffer is full and send them again when the buffer is empty.
  - If the unit tries to cache instructions when it has already cached another, then the last one will be overwritten. But usually you don't need to worry about this problem.
  - Waypoint mode is not currently supported. This might be changed in the future.
...
```{warning}
For mouse clicks, this feature will cache the actions generated by the mouse click. This means that the unit may therefore perform some actions that are not currently allowed (for example, in this frame a HTNK caches a command to enter the tank bunker, but in the next frame it is parasitized. At this time, the HTNK will not be able to issue instructions to enter the tank bunker, but the cache can). These situations are usually harmless, but we cannot promise to it. So please use it at your own risk.
```
