...
- Fixed units, buildings and tibeirums not being affected by map lighting color tint (must be enabled with `FixLightingTint=true` under `[General]` in `rulesmd.ini`).
