...
- Fixed the bug that restored cyborg legs when the soldier left transport. Old behavior can be enabled back by setting `rulesmd.ini -> [SOMETECHNO] -> FixEnteringCyborgLegs=true` for a transport.
