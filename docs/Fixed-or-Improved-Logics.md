...
- Fixed a bug where passengers created by the InitialPayload logic would fail to fire when the transport unit with `OpenTopped=yes` moved to an area that the passengers' `MovementZone` cannot move into.
