...
- Pips for TechnoTypes with `Spawns` can now also be customized.
  ...
  - `SpawnsPip` and `EmptySpawnsPip` are frames (zero-based) of `pips.shp` (for buildings) or `pips2.shp` (for others) used for a spawnee pip and empty spawnee pip, respectively.
...
- Pips for `Storage` can now also be customized via new keys in `[AudioVisual]`.
  - `Pips.Tiberiums` can be used to list frames (zero-based) of `pips.shp` (for buildings) or `pips2.shp` (for others) used for tiberium types, in the listed order corresponding to tiberium type index. Defaults to 5 for tiberium type index 1, otherwise 2.
...
Pips.Tiberiums=2,5,2,2               ; list of integers, frames of pips.shp (buildings) or pips2.shp (others) (zero-based)
...
SpawnsPip=1                          ; integer, frame of pips.shp (buildings) or pips2.shp (others) (zero-based)
EmptySpawnsPip=0                     ; integer, frame of pips.shp (buildings) or pips2.shp (others) (zero-based)
