...
- Warheads can now apply additional chance-based damage or Warhead detonation ('critical hits') with the ability to customize chance, damage, affected targets, affected target HP threshold and animations of critical hit.
  - `Crit.Chance(s)` determines chance for a critical hit to occur. By default this is checked once when the Warhead is detonated and every target that is susceptible to critical hits will be affected. If `Crit.ApplyChancePerTarget` is set, then whether or not the chance roll is successful is determined individually for each target. Can be set to a single value for all conditions, or a list of values which would work when target health is below certain percentage. Value from position matching the position from `Crit.AffectBelowPercent(s)` is used if found, or 0.0 if not found. If both are set, `Crit.Chances` will override `Crit.Chance`.
  - `Crit.ExtraDamage(s)` determines the damage dealt by the critical hit. If `Crit.Warhead` is set, the damage is used to detonate the specified Warhead on each affected target, otherwise the damage is directly dealt based on current Warhead's `Verses` settings. Can be set to a single value for all conditions, or a list of values which would work when target health is below certain percentage. Value from position matching the position from `Crit.AffectBelowPercent(s)` is used if found, or 0 if not found. If both are set, `Crit.ExtraDamages` will override `Crit.ExtraDamage`.
  ...
  - `Crit.AffectBelowPercent(s)`, if set to a single value, determines minimum percentage of their maximum `Strength` that targets must have left to be affected by a critical hit. If set to a list of values, it'll further determine `Crit.Chance(s)` and `Crit.ExtraDamage(s)` when target health is below the certain percentage listed here. If both are set, `Crit.AffectBelowPercents` will override `Crit.AffectBelowPercent`.
