...
- Warheads can now apply additional chance-based damage or Warhead detonation ('critical hits') with the ability to customize chance, damage, affected targets, affected target HP threshold and animations of critical hit.
  ...
  - `Crit.Affects` can be used to customize types of targets that this Warhead can deal critical hits against. Critical hits will not occur on empty cells unless `Crit.Warhead` is set and this list contains `all` or `empty` as values.
