...
0. Phobos requires [SyringeEx](https://github.com/Phobos-developers/SyringeEx) - an extended, open-source version of Syringe - to run; the game will show an error and quit on startup under older Syringe versions. Download `Syringe.exe` from the [SyringeEx releases page](https://github.com/Phobos-developers/SyringeEx/releases) and drop it into your game folder (where your `gamemd.exe` is located), replacing any existing `Syringe.exe` (for example the one shipped with Ares). It's highly recommended to **install [Ares](https://launchpad.net/ares/+download)** too to get full Phobos feature set, just drop all the files from the archive except documentation folder and `Syringe.exe` into your game folder.
