...
void TechnoTypeExt::ExtData::LoadFromINIFile(CCINIClass* const pINI)
{
	...
	this->LoadFromINIByWhatAmI(pINI, exINI, pSection, exArtINI, pArtSection, pThis);
...
void TechnoTypeExt::ExtData::LoadFromINIByWhatAmI(CCINIClass* const pINI, INI_EX& exINI, const char* pSection, INI_EX& exArtINI, const char* pArtSection, TechnoTypeClass* pThis)
{
	this->AbsType = pThis->WhatAmI();
	char tempBuffer[40];
	...
	case AbstractType::UnitType:
	{
		...
		if (pThis->Gunner)
		{
			...
			this->WeaponGroupAs.resize(weaponCount);
