...
DEFINE_HOOK(0x469EC0, BulletClass_Logics_AirburstWeapon, 0x6)
{
	...
	if ((pType->Airburst || pTypeExt->Splits) && pWeapon)
	{
		...
		{
			const float cellSpread = static_cast<float>(pTypeExt->Splits_TargetingDistance.Get()) / (float)Unsorted::LeptonsPerCell;
			...
			auto const& technos = Helpers::Alex::getCellSpreadItemsExt(coordsTarget, cellSpread, true, false);
