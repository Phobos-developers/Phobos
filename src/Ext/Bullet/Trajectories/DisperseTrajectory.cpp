...
bool DisperseTrajectory::OnAI(BulletClass* pBullet)
{
	...
	if (this->WeaponCount != 0 && (this->WeaponScope == 0 || pBullet->TargetCoords.DistanceFrom(pBullet->Location) <= this->WeaponScope) && (!pOwner || PrepareDisperseWeapon(pBullet, pOwner)))
		return true;

	if (this->UniqueCurve ? CurveVelocityChange(pBullet) : NotCurveVelocityChange(pBullet, pOwner))
		return true;
...
bool DisperseTrajectory::BulletRetargetTechno(BulletClass* pBullet, HouseClass* pOwner)
{
		...
		for (CellSpreadEnumerator ThisCell(static_cast<size_t>(this->RetargetRadius + 0.99)); ThisCell; ++ThisCell)
		{
			if (const CellClass* const pCell = MapClass::Instance->GetCellAt(*ThisCell + CellClass::Coord2Cell(RetargetCoords)))
			{
				...
				while (pObject)
				{
					...
					if (!pTechno || pTechno->GetHeight() > 200 || CheckTechnoIsInvalid(pTechno))
						continue;
	...
	else
	{
		...
		{
			if (pTechno->GetHeight() <= 200 || CheckTechnoIsInvalid(pTechno))
				continue;
	...
	HouseClass* pOwner, WarheadTypeClass* pWH, AbstractClass* pTarget)
{
	...
	{
		if (this->TargetInTheAir != pTechno->GetHeight() > 200 || CheckTechnoIsInvalid(pTechno))
			continue;
