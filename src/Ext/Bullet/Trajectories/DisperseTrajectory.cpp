...
bool DisperseTrajectoryType::Load(PhobosStreamReader& Stm, bool RegisterForChange)
{
	...
	Stm
		...
		.Process(this->RateofTurning, false)
...
void DisperseTrajectoryType::Read(CCINIClass* const pINI, const char* pSection)
{
	...
	this->RateofTurning.Read(exINI, pSection, "Trajectory.Disperse.RateofTurning");
...
void DisperseTrajectory::OnUnlimbo(BulletClass* pBullet, CoordStruct* pCoord, BulletVelocity* pVelocity)
{
	...
	this->RateofTurning = pType->RateofTurning > 0.1 ? pType->RateofTurning : 0.1;
...
bool DisperseTrajectory::OnAI(BulletClass* pBullet)
{
	...
	{
		if (BulletRetargetTechno(pBullet, pOwner))
			return true;

		if (this->InStraight || pBullet->SourceCoords.DistanceFrom(pBullet->Location) >= this->PreAimCoord.Magnitude())
			StandardVelocityChange(pBullet);
...
void DisperseTrajectory::CurveVelocityChange(BulletClass* pBullet)
{
	ObjectClass* pTarget = abstract_cast<ObjectClass*>(pBullet->Target);
	...
	else
	{
		...
		this->LastTargetCoord = pBullet->TargetCoords;
		ChangeBulletVelocity(pBullet, TargetLocation, 24.0, true);
	}
}

void DisperseTrajectory::StandardVelocityChange(BulletClass* pBullet)
{
	ObjectClass* pTarget = abstract_cast<ObjectClass*>(pBullet->Target);
	CoordStruct TargetLocation = pTarget ? pTarget->GetCoords() : pBullet->TargetCoords;
	pBullet->TargetCoords = TargetLocation;
	...
	this->LastTargetCoord = pBullet->TargetCoords;
	double TurningRadius = this->RateofTurning * this->LaunchSpeed * this->LaunchSpeed / 16384;
	ChangeBulletVelocity(pBullet, TargetLocation, TurningRadius, false);
}

void DisperseTrajectory::ChangeBulletVelocity(BulletClass* pBullet, CoordStruct TargetLocation, double TurningRadius, bool Curve)
{
...
void DisperseTrajectory::CreateDisperseBullets(BulletClass* pBullet, WeaponTypeClass* pWeapon, AbstractClass* BulletTarget, HouseClass* pOwner)
{
	...
	if (pWeapon->IsElectricBolt)
	{
		if (EBolt* pEBolt = GameCreate<EBolt>())
		{
			...
			if (auto pWeaponTypeExt = WeaponTypeExt::ExtMap.Find(pWeapon))
				if (pWeaponTypeExt->Bolt_Disable1)
