...
void TechnoExt::ExtData::ApplyInterceptor()
{
	...
	if (pTypeExt->InterceptorType && !pThis->Target && !pThis->RearmTimer.HasTimeLeft() && !this->IsBurrowed)
	{
			...
			else if (this->PassengerDeletionTimer.Completed()) // Execute only if timer has ran out after being started
			{
				...
				if (pTransportType->Gunner)
				{
					if (auto const pFoot = abstract_cast<FootClass*, true>(pThis))
					{
						...
						if (pThis->Passengers.NumPassengers > 0)
							pFoot->ReceiveGunner(pLastPassenger ? abstract_cast<FootClass*, true>(pLastPassenger) : pPreviousPassenger);
					}
				}
