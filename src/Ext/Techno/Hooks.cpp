...
static bool IsChasing(FootClass* pThis)
{

}

static bool IsFiring(UnitClass* pThis)
{

}

static bool IsFiring(InfantryClass* pThis)
{

}

#pragma region ChasingEnhancement

DEFINE_HOOK(0x521277, InfantryClass_ProcessMovingSequence_WalkSequence, 0x6)
{
	GET(InfantryClass*, pThis, EBP);
	return pThis->IsFiring || pThis->RearmTimer.InProgress() ? 0x521292 : 0;
}

DEFINE_HOOK(0x521228, InfantryClass_ProcessMovingSequence_WalkSequence1, 0x6)
{
	GET(InfantryClass*, pThis, EBP);
	return pThis->IsFiring || pThis->RearmTimer.InProgress() ? 0x52130D : 0;
}

//DEFINE_HOOK(0x520D52, InfantryClass_ProcessSequence_WalkSequence, 0x6)
//{
//	GET(InfantryClass*, pThis, ESI);
//	return pThis->IsFiring || pThis->RearmTimer.InProgress() ? 0x520E52 : 0;
//}

//DEFINE_HOOK(0x520E26, InfantryClass_ProcessSequence_WalkSequence1, 0x6)
//{
//	GET(InfantryClass*, pThis, ESI);
//	return pThis->IsFiring || pThis->RearmTimer.InProgress() ? 0x520E52 : 0;
//}

DEFINE_HOOK(0x75BC8E, WalkLocomotionClass_ProcessMove_DoTurn, 0x7)
{
	GET(LocomotionClass*, pLoco, EBP);
	auto pThis = pLoco->LinkedTo;
	return pThis->IsFiring || pThis->RearmTimer.InProgress() ? 0x75BC9A : 0;
}

DEFINE_HOOK(0x75C02C, WalkLocomotionClass_ProcessMove_DoTurn1, 0x7)
{
	GET(LocomotionClass*, pLoco, EBP);
	auto pThis = pLoco->LinkedTo;
	return pThis->IsFiring || pThis->RearmTimer.InProgress() ? 0x75C038 : 0;
}

DEFINE_HOOK(0x51AC16, InfantryClass_SetDest_Sequence, 0x5)
{
	GET(InfantryClass*, pThis, EBP);
	return pThis->IsFiring || pThis->RearmTimer.InProgress() ? 0x51AC25 : 0;
}

DEFINE_HOOK(0x52092A, InfantryClass_UpdateFiring_StopMoving, 0x6)
{
	GET(InfantryClass*, pThis, EBP);
	auto crd = MapClass::Instance.GetCellAt(pThis->Target->GetDestination())->GetCoords();
	pThis->MoveTo(&crd);
	return 0x52094C;
}

DEFINE_HOOK_AGAIN(0x4D6547, FootClass_ApproachTarget_CheckRange, 0x5);
DEFINE_HOOK(0x4D6223, FootClass_ApproachTarget_CheckRange, 0x5)
{
	GET(FootClass*, pThis, EBX);
	GET_STACK(bool, isAirUnit, STACK_OFFSET(0x158, -0x13D));
	AbstractClass* targetToApproach = pThis->Target;
	auto movableTarget = abstract_cast<FootClass*>(pThis->Target);
	...
	do
	{
		...
		auto carefullyCheckDist = 3 * Unsorted::LeptonsPerCell;
		auto directDist = pThis->DistanceFrom(movableTarget);
		if (directDist >= carefullyCheckDist)
			break;

		//auto pathingDist = AStarClass::Instance.AttemptPath(pThis->GetMapCoords(&buffer), movableTarget->GetMapCoords(&buffer), pThis, pThis->IsOnBridge(), movableTarget->IsOnBridge(), MovementZone::None);
		//if (pathingDist >= carefullyCheckDist)
		//	break;

		auto targetLoco = movableTarget->Locomotor;
		if (!targetLoco.GetInterfacePtr()->Is_Moving_Now())
			break;

		auto teleportLoco = locomotion_cast<TeleportLocomotionClass*>(targetLoco);
		if (teleportLoco)
			break;

		if (movableTarget->GetCoords() == movableTarget->GetDestination())
			break;

		auto offset = movableTarget->GetDestination() - movableTarget->GetCoords();
		auto dist = offset.Magnitude();
		auto newOffset = offset + CoordStruct(256.0 * offset.X / dist, 256.0 * offset.Y / dist, 0);
		auto destCrd = movableTarget->GetCoords() + newOffset;
		targetToApproach = MapClass::Instance.GetCellAt(destCrd);
		//auto getRealSpeed = [&](ILocomotionPtr loco) -> double
		//	{
		//		bool isDrive = locomotion_cast<DriveLocomotionClass*>(loco);
		//		return movableTarget->Locomotor.GetInterfacePtr()->Apparent_Speed() * (isDrive ? 1.6 : 1.0);
		//	};
		//auto targetSpeed = getRealSpeed(targetLoco);
		//auto thisSpeed = getRealSpeed(pThis->Locomotor);
		//auto leadTime = 
	}
	while (false);
	R->EDX(targetToApproach);
	return 0;
}

DEFINE_HOOK_AGAIN(0x4D65AA, FootClass_ApproachTarget_MarkUp, 0x9);
DEFINE_HOOK(0x4D62DA, FootClass_ApproachTarget_MarkUp, 0x9)
{
	GET(FootClass*, pThis, EBX);
	if (auto pFootTarget = abstract_cast<FootClass*>(pThis->Target))
	{
		pFootTarget->Mark(MarkType::Up);
	}
	return 0;
}

DEFINE_HOOK(0x4D6653, FootClass_ApproachTarget_MarkDown1, 0x5)
{
	GET(FootClass*, pThis, EBX);
	if (auto pFootTarget = abstract_cast<FootClass*>(pThis->Target))
	{
		pFootTarget->Mark(MarkType::Down);
	}
	return 0x4D66EC;
}

DEFINE_HOOK(0x4D6383, FootClass_ApproachTarget_MarkDown2, 0x5)
{
	GET(FootClass*, pThis, EBX);
	if (auto pFootTarget = abstract_cast<FootClass*>(pThis->Target))
	{
...
//namespace CalcDirContext
//{
//	CoordStruct crd;
//}
//
//DEFINE_HOOK(0x4D5C53, FootClass_ApproachTarget_CalcDir, 0x5)
//{
//	GET(FootClass*, pThis, EBX);
//	CalcDirContext::crd = MapClass::Instance.GetCellAt(pThis->GetCoords())->GetCoords();
//	R->EAX(&CalcDirContext::crd);
//	return 0;
//}
//
//DEFINE_HOOK(0x51C553, InfantryClass_IsCellOccupied_Chase, 0x5)
//{
//	GET(InfantryClass*, pThis, EBP);
//	GET(ObjectClass*, pObject, ESI);
//	return pThis->Target && pThis->Type->CloseRange && pThis->Target == pObject ? 0x51C77F : 0;
//}
