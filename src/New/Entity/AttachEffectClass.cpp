...
bool AttachEffectClass::ShouldBeDiscardedNow()
{
	...
	if (pType->DiscardOn_CumulativeCount > 0 && pType->DiscardOn_CumulativeCount <= TechnoExt::ExtMap.Find(pTechno)->GetAttachedEffectCumulativeCount(pType))
	{
		this->LastDiscardCheckValue = true;
		return true;
	}
	...
	{
		const bool isMoving = pFoot->Locomotor->Is_Really_Moving_Now();
		...
		if (!isMoving && (discardOn & DiscardCondition::Stationary) != DiscardCondition::None)
		{
			this->LastDiscardCheckValue = true;
			return true;
		}
