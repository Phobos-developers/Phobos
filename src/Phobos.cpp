...
const wchar_t* Phobos::VersionString = L"Phobos nightly build (" STR_GIT_COMMIT L" @ " STR_GIT_BRANCH L"). DO NOT SHIP IN MODS!";
#elif !defined(IS_RELEASE_VER)
const wchar_t* Phobos::VersionString = L"Phobos development build #" str(BUILD_NUMBER) L". Please test the build before shipping.";
#else
const wchar_t* Phobos::VersionString = L"Phobos release build v" FILE_VERSION_STR L".";
#endif
...
DEFINE_HOOK(4F4583, GScreenClass_DrawText, 6)
{
	...
	{
		auto wanted = Drawing::GetTextDimensions(Phobos::VersionString);
		...
		DSurface::Composite->DrawTextA(Phobos::VersionString, rect.X + 5, 5, COLOR_RED);
	}
	return 0;
}
#endif
